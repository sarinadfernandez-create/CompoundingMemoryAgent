...
TEST_CUSTOMER = {
    ...
    "reference": {
        "actions": [
            "Incorporate as a Delaware C-Corp or LLC (no US citizenship required for ownership)",
            "Apply for EIN via Form SS-4 international procedure (no SSN required)",
            "Arrange a US registered agent and US virtual mailing address",
            "Open a business bank account at a non-resident-friendly bank (Mercury, Brex, Wise)",
            "Migrate existing US client invoicing under the new entity",
            "Backdated accounting and tax review for prior personal-wire revenue",
            "Engage a cross-border US-India tax advisor for treaty and double-taxation guidance",
        ],
        "questions": [
            "How are existing US clients contracting and paying — personal name or business name",
            "Plans to obtain a US ITIN",
            "Revenue collected via personal wires to date (for back-tax assessment)",
            "Will you be physically working from India indefinitely",
            "Any existing Indian entity or GST registration to coordinate with",
        ],
        "risks": [
            "Banking access friction for non-residents — many US banks require physical presence; plan around Mercury / Brex / Wise",
            "US-India tax treaty implications and double-taxation exposure — cross-border tax advisor required",
            "Prior personal-wire revenue may require amended personal returns and back-taxes; possible self-employment tax exposure",
            "US clients may need updated contracts and invoices under the new entity for audit and 1099 reasons",
            "FATCA reporting and Indian foreign-bank reporting once US accounts are opened",
        ],
    },
}
...
GRADE_SYSTEM = (
    "You are evaluating how well a draft onboarding plan covers a reference of "
    "what thorough coverage requires. For each reference item across actions, "
    "questions, and risks, decide whether the draft covers it (loose match — "
    "synonyms, paraphrases, and slightly broader phrasings count as covered). "
    "Every reference item must appear in EXACTLY ONE of 'covered' or 'missing'.\n\n"
...
def call(system, user, max_tokens=4096):
    # 4096 by default — thorough plans against a multi-section playbook can
    # easily push past 2k tokens of JSON output, so leave plenty of headroom.
...
def make_plan(message, playbook):
    ...
    Retries once with a bigger token budget if the first response is truncated /
    fails to parse, so a parse error never silently becomes an empty plan."""
...
def grade(draft, reference):
    ...
    user = (f"Reference plan (what thorough coverage requires):\n"
...
def main():
    ...
    for ep, c in enumerate(CUSTOMERS, 1):
        ...
        with open(f"Iter4-memory_ep{ep}.md", "w") as fh:
            fh.write(playbook)
    ...
    # ---------- Held-out test: COLD vs WARM with graded recall -----------
    print("\n" + "=" * 72)
    print(f"HELD-OUT TEST CUSTOMER — {TEST_CUSTOMER['summary']}")
    print("=" * 72)
    print(f"\nCustomer message:\n  {TEST_CUSTOMER['message']}\n")

    # COLD: same model, NO playbook. Establishes the baseline.
    print("─" * 72)
    print("RUN 1: COLD  (no playbook — what the model knows on its own)")
    print("─" * 72)
    cold_plan = make_plan(TEST_CUSTOMER["message"], "")
    print(json.dumps(cold_plan, indent=2))
    cold_g = grade(cold_plan, TEST_CUSTOMER["reference"])
    cold_total = len(cold_g["covered"]) + len(cold_g["missing"])
    print(f"\n  Coverage of reference: {cold_g['recall']*100:.0f}% "
          f"({len(cold_g['covered'])}/{cold_total} items)")

    # WARM: same model, full self-built playbook.
    print("\n" + "─" * 72)
    print("RUN 2: WARM  (with the self-built playbook from 5 prior cases)")
    print("─" * 72)
    warm_plan = make_plan(TEST_CUSTOMER["message"], playbook)
    print(json.dumps(warm_plan, indent=2))
    warm_g = grade(warm_plan, TEST_CUSTOMER["reference"])
    warm_total = len(warm_g["covered"]) + len(warm_g["missing"])
    print(f"\n  Coverage of reference: {warm_g['recall']*100:.0f}% "
          f"({len(warm_g['covered'])}/{warm_total} items)")

    # Headline: the quantified improvement.
    print("\n" + "=" * 72)
    print("QUANTIFIED IMPROVEMENT (held-out case, same frozen model)")
    print("=" * 72)
    delta_pp = (warm_g["recall"] - cold_g["recall"]) * 100
    print(f"  Cold   : {cold_g['recall']*100:5.0f}%  coverage of reference")
    print(f"  Warm   : {warm_g['recall']*100:5.0f}%  coverage of reference")
    print(f"  Δ      : {delta_pp:+5.0f} percentage points  (purely from self-built memory)")
    if warm_g["missing"]:
        print("\n  What the warm plan still misses:")
        for m in warm_g["missing"]:
            print(f"    • [{m.get('kind','?')}] {m.get('ref_item','')}")

    with open("Iter4-memory.md", "w") as fh:
        fh.write(playbook)
    print("\nSaved final playbook to Iter4-memory.md (per-episode snapshots in Iter4-memory_ep1.md … Iter4-memory_ep5.md)")
