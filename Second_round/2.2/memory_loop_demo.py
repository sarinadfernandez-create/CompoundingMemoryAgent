...
ops task types. The held-out evaluation lives in eval_harness.py.

USAGE
-----
    python memory_loop_demo.py --iter v1_baseline

Each iteration writes its artifacts to ./artifacts/{iter}/ so you can keep
multiple runs side by side and compare across iterations.

Outputs (under ./artifacts/{iter}/):
...
# =============================================================================
# HELIX LABS canon (training cases)
# =============================================================================
CASES = [
    {
        "id": "T1",
...
def parse_sections(playbook_md):
    """Parse markdown playbook into {section_name: [bullet_lines, ...]}."""
    sections = {}
    current = None
...
# =============================================================================
def parse_args():
    p = argparse.ArgumentParser(description="Train the Helix Labs memory playbook.")
    p.add_argument(
        "--iter", required=True,
        help="Iteration name (e.g., 'v1_baseline', 'v2_bullet_rag'). "
