"""
eval_harness.py  (v3 — cascade-ready)
--------------------------------------
...
V3 ADDITIONS:
  ...
  - Grader stays on a single fixed model (default Sonnet) for stable scoring
    regardless of which planner model a config uses

USAGE
-----
    python eval_harness.py --iter v3_cascade
    python eval_harness.py --iter v3_cascade --configs warm_rag warm_rag_cascade
...
--------------
  ...
  warm_rag_cascade  : bullet RAG + HAIKU planner   ← NEW

METRICS — unchanged from v2 plus per-config model annotation in summary.
"""
...
# =============================================================================
# CONFIGURATIONS  — each returns (plan, usage, extras, planner_model)
# =============================================================================
...
def run_warm_routed(test_case, playbook, seed, iter_dir):
    ...
    # Combine usage from router + planner (different models — sum costs in $)
    usage = {
...
def run_warm_rag(test_case, playbook, seed, iter_dir):
    """Bullet RAG with Sonnet planner — same as v2."""
...
def run_warm_rag_cascade(test_case, playbook, seed, iter_dir):
    """Bullet RAG with HAIKU planner — cost-optimized cascade.

    Hypothesis: a smaller model can still produce a thorough plan as long as
    the bullet-level RAG surfaces the right Helix-specific facts. Most of the
    'work' shifts from reasoning to lookup.
    """
...
# =============================================================================
# METRIC COMPUTATION — unchanged + adds planner_model passthrough
# =============================================================================
...
def print_summary(results, n_seeds, iter_name, config_names):
    ...
    # Cost-quality table (the cascade story)
    lines.append("\n" + "=" * 100)
    lines.append(f"CASCADE STORY: cost vs quality across configs (avg across tests)")
    lines.append("=" * 100)
    lines.append(f"  {'Config':<18} {'Avg recall':>12} {'Avg cost/run':>14} {'$/recall point':>16} {'vs warm_full':>14}")
    lines.append(f"  {'-' * 18} {'-' * 12} {'-' * 14} {'-' * 16} {'-' * 14}")

    # Compute averages across tests for each config
    avg_by_config = {}
    for cfg in config_names:
        recalls = []
        costs = []
...
def main():
    ...
    if ("warm_rag" in selected_configs or "warm_rag_cascade" in selected_configs) \
            and _openai_client is None:
    ...
    with open(f"{iter_dir}/eval_results.json", "w") as fh:
        json.dump({
            ...
            "config": {
                ...
                "top_k_rag": TOP_K if ("warm_rag" in config_names or "warm_rag_cascade" in config_names) else None,
                "embed_model": EMBED_MODEL if ("warm_rag" in config_names or "warm_rag_cascade" in config_names) else None,
            },
            "playbook_stats": {
